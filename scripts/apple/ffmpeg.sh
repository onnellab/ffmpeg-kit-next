...
# UPDATE BUILD FLAGS
export CFLAGS="${ARCH_CFLAGS} ${APP_CFLAGS} ${COMMON_CFLAGS} ${OPTIMIZATION_CFLAGS} ${MIN_VERSION_CFLAGS}${FFMPEG_CFLAGS} ${COMMON_INCLUDES} ${FFMPEG_KIT_DSYM_FLAGS:-}"
export CXXFLAGS=$(get_cxxflags "${LIB_NAME}")
export LDFLAGS="${ARCH_LDFLAGS}${HIGH_PRIORITY_LDFLAGS}${FFMPEG_LDFLAGS} ${LINKED_LIBRARIES} ${COMMON_LDFLAGS} ${BITCODE_FLAGS} ${OPTIMIZATION_FLAGS} ${FFMPEG_KIT_DSYM_FLAGS:-}"
...
if [[ -n ${FFMPEG_KIT_DSYM_BUILD:-} ]]; then
  for FFMPEG_LIB in "${FFMPEG_LIBS[@]}"; do
    "${BASEDIR}/tools/apple/capture-thin-dsym.sh" \
      "${BASEDIR}/src/ffmpeg/${FFMPEG_LIB}/${FFMPEG_LIB}.$(get_ffmpeg_library_major_version "${FFMPEG_LIB}").dylib" \
